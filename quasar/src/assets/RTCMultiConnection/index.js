...
class Negotiating {
  ...
  async waitNegotiating(id) {
    return new Promise(async (resolve, reject) => {
      const check = () => !!this.signals[id]
      let attemps = 10
      while(attemps) {
        if (!check()) {
          resolve()
...
var RTCMultiConnection = function(roomid, forceOptions) {
    const negotiating = new Negotiating();
    ...
    function MultiPeers(connection) {
        ...
        this.onNegotiationNeeded = function(message, remoteUserId) {
          console.log('onNegotiationNeeded', message, remoteUserId)
        };
        this.addNegotiatedMessage = async function(message, remoteUserId) {
            const { type, candidate, enableMedia, readyForOffer } = message
            console.log('addNegotiatedMessage', type, candidate, enableMedia, readyForOffer)
            if (message.type && message.sdp) {
                if (message.type == 'answer') {
                    // await negotiating.waitNegotiating(remoteUserId)
                    if (connection.peers[remoteUserId]) {
                        connection.peers[remoteUserId].addRemoteSdp(message, () => negotiating.resetNegotiating(remoteUserId));
                    }
                }
    ...
    function PeerInitiator(config) {
        ...
        this.addRemoteSdp = function(remoteSdp, cb) {
            ...
            const { connectionState, iceConnectionState, iceGatheringState, signalingState } = peer
            console.log('addRemoteSdp', connectionState, iceConnectionState, iceGatheringState, signalingState)
    ...
    (function(connection) {
        ...
        mPeer.onNegotiationNeeded = async function(message, remoteUserId, callback) {
            callback = callback || function() {};
